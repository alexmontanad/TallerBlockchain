...
class Bloque:
    ...
    def __init__(self, hash_anterior, codigo, etapa_actual, observacion, rsa_interventor):
        ...
        self.observaciones = []
        self.agregar_observacion(observacion, rsa_interventor)
...
class Blockchain:
    ...
    def _crear_bloque_genesis(self):
        """Crea bloque génesis."""
        hash_anterior = "0" * 128
        codigo_inicial = f"PROYECTO: {self.nombre_proyecto} - INICIO"
        ...
        bloque_genesis = Bloque(
            ...
            etapa_actual=0,
            observacion="Acta de inicio del contrato. Proyecto aprobado para desarrollo.",
    ...
    def agregar_etapa(self, codigo, etapa_actual, observacion):
        """Agrega nueva etapa."""
        ...
        for i, bloque in enumerate(self.cadena):
            ...
            for j in range(len(bloque.observaciones)):
                if not bloque.verificar_firma(j, self.rsa_interventor):
                    return False, f"Bloque {i}: firma RSA-512 inválida"
    ...
    def mostrar_cadena(self):
        ...
        print(f"\n{'#'*70}")
        print(f"BLOCKCHAIN: {self.nombre_proyecto}")
        print(f"Total de bloques: {len(self.cadena)}")
        print(f"{'#'*70}")
    ...
    def mostrar_datos_descifrados(self, indice_bloque):
        """Muestra datos descifrados de un bloque."""
        if indice_bloque >= len(self.cadena):
            print(f"✗ Bloque {indice_bloque} no existe")
            return
        
        bloque = self.cadena[indice_bloque]
        print(f"\n{'='*70}")
        print(f"DATOS DESCIFRADOS - BLOQUE {indice_bloque}")
        print(f"{'='*70}")
        
        datos = bloque.descifrar_bloque()
        print(json.dumps(datos, indent=2, ensure_ascii=False))
    
    def intentar_modificacion(self, indice):
        """Simula modificación fraudulenta."""
        print(f"\n{'!'*70}")
        print(f"SIMULACIÓN: Modificación fraudulenta del bloque {indice}")
        print(f"{'!'*70}")
        ...
        print(f"\n⚠️  Atacante modifica el código del bloque...")
        
        bloque.codigo = "CÓDIGO_FRAUDULENTO_INYECTADO"
        bloque.hash_codigo = bloque._calcular_hash_codigo()
        nuevo_hash = bloque._calcular_hash_actual()
        
        print(f"Nuevo hash: {nuevo_hash[:32]}...")
        print(f"\n🔍 Sistema validando integridad...")
        
        valida, mensaje = self.validar_cadena()
        
        if not valida:
            print(f"\n✗ FRAUDE DETECTADO: {mensaje}")
            print(f"✓ La blockchain rechazó la modificación")
            print(f"✓ Sistema de seguridad funcionando correctamente")
            self._notificar("ALERTA: Intento de modificación detectado y bloqueado", tipo="warning")
        else:
            print(f"\n⚠️  ALERTA: Fraude no detectado (error en el sistema)")
...
def demo_completa():
    ...
    print("\n" + "-"*70)
    print("DEMOSTRACIÓN: DESCIFRADO AES DE BLOQUE")
    print("-"*70)
    bc.mostrar_datos_descifrados(2)
    
    bc.intentar_modificacion(2)
    
    print("\n" + "-"*70)
    print("VALIDACIÓN POST-INTENTO DE FRAUDE")
    print("-"*70)
    valida, mensaje = bc.validar_cadena()
    print(f"\nResultado: {mensaje}")
